...
Now (in the editor of your choice) define the `foo` type to be used as a type place-holder. And then write three `jig:template` code fragments that use this `foo` type as a placeholder.
```go
package stack
...
```
Run the code with the *go* tool to see it fail:
```bash
$ go run *.go
...
```
Run the code again with the *go* tool to see it succeed:
```bash
go run *.go
Hello, World!
$ 
...
By the way, *jig* is fully capable of specializing templates in terms of `interface{}` as opposed to specific types. Using `Stack` instead of `StringStack`:
```go
package main
...
```
*Jig* will actually generate a stack in terms of the `interface{}` type. *Jig* internally maps the absense of a reference type name to indicate `interface{}` as the actual type to use.
```go
// Code generated by jig; DO NOT EDIT.
