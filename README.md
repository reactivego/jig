...
The `jig` command generates Go code by expanding templates from a generics library. A template is a piece of generic code with type place-holders. Template expansion then replaces type place-holders with concrete types.
...
$GOPATH/src/github.com/reactivego/jig/example/stack
                                              └── generic
                                                  └── test
```
We see three levels here; `stack` at the root package level, `generic` and `test`. The root folder `stack` exports our library specialized on the type `interface{}`. The `generic` folder contains the actual generics library and the `test` folder contains tests code that exercises the generics library. Code is generated by *jig* in the `stack` folder and in the `test` folder. In both cases, by expanding templates from the `generic` folder.
...
The `test` folder contains test code for testing all aspects of the generics library. The code here should exercise the generics library using different types. Running *jig* in the `test` folder will generate code into the `stack.go` file. 
